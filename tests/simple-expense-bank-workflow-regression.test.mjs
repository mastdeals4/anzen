...
test('cancelled expenses are historical view-only records without payment-voucher shortcut', () => {
  assert.match(expenseUi, /lifecycleFilter/);
  assert.match(expenseUi, /effective_posting_state === 'REVERSED'/);
  assert.match(expenseUi, /filter\.value as 'operational' \| 'cancelled'/);
  assert.doesNotMatch(expenseUi, /title="Settle via Payment Voucher"/);
  const actionBlock = expenseUi.slice(expenseUi.indexOf('<FinanceActionButton\n                            action="view"'), expenseUi.indexOf('{isAdmin && postingState === \'PENDING\''));
  assert.match(actionBlock, /postingState === 'ACTIVE' \|\| postingState === 'PENDING' \|\| postingState === 'REJECTED'/);
  assert.doesNotMatch(actionBlock, /postingState === 'REVERSED'/);
});
