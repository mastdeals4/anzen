  ...
  >
    ...
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-2">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        <label className="relative xl:col-span-2"><Search className="absolute left-2 top-2 h-3.5 w-3.5 text-gray-400" /><input name="filter" aria-label="Search voucher, journal, party, problem…" value={filter('search')} onChange={event => setFilter('search', event.target.value)} placeholder="Search voucher, journal, party, problem…" className="w-full rounded-md border border-gray-300 bg-white py-1.5 pl-7 pr-2 text-xs" /></label>
        <select name="filter" aria-label="Filter" value={filter('type')} onChange={event => setFilter('type', event.target.value)} className="rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs"><option value="">All document types</option>{types.map(value => <option key={value} value={value}>{humanize(value)}</option>)}</select>
        <select name="filter" aria-label="Filter" value={filter('status')} onChange={event => setFilter('status', event.target.value)} className="rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs"><option value="">All statuses</option>{statuses.map(value => <option key={value} value={value}>{humanize(value)}</option>)}</select>
        <select name="filter" aria-label="Filter" value={filter('currency')} onChange={event => setFilter('currency', event.target.value)} className="rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs"><option value="">All currencies</option>{currencies.map(value => <option key={value} value={value}>{value}</option>)}</select>
        <label className="flex items-center gap-1 text-[11px] text-gray-500">From<input name="filter" aria-label="Filter" type="date" value={filter('from')} onChange={event => setFilter('from', event.target.value)} className="min-w-0 flex-1 rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs" /></label>
        <label className="flex items-center gap-1 text-[11px] text-gray-500">To<input name="filter" aria-label="Filter" type="date" value={filter('to')} onChange={event => setFilter('to', event.target.value)} className="min-w-0 flex-1 rounded-md border border-gray-300 bg-white px-2 py-1.5 text-xs" /></label>
      </div>
    </div>
    <div className="overflow-x-auto rounded-lg border border-gray-200"><table className="min-w-[1700px] divide-y divide-gray-200 text-xs"><thead className="bg-gray-50 text-left font-medium uppercase tracking-wide text-gray-500"><tr><th className="w-8 px-2 py-2" />{['Date','Voucher Number','Journal Number','Amount','Bank','From Bank','To Bank','Customer / Supplier','Document Type','Current Category','Current GL Account','Problem','Status','Correction'].map(label => <th key={label} className="px-2 py-2">{label}</th>)}</tr></thead><tbody className="divide-y divide-gray-200 bg-white">
      {filteredRows.map(row => { const isExpanded = expanded.has(row.row_id); const edit = edits[row.row_id] || {};
        ...
        {isExpanded && <tr key={`${row.row_id}:details`} className="bg-gray-50/70"><td colSpan={15} className="px-4 py-3"><div className="mb-3 grid gap-2 rounded-md border border-gray-200 bg-white p-3 sm:grid-cols-2 lg:grid-cols-5">{[['Voucher Number',row.voucher_number],['Journal Number',row.journal_number],['Date',row.date],['Amount',formatAmount(row.amount,row.currency)],['Currency',row.currency],['Bank Alias',row.bank_alias || row.bank],['Current GL',row.current_gl_account],['Current Category',humanize(row.current_category)],['Current Subcategory',humanize(row.current_subcategory)],['Source Document',row.current_source_document]].map(([label,value]) => <div key={label}><div className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">{label}</div><div className="mt-0.5 break-words text-xs font-medium text-gray-800">{value || '—'}</div></div>)}</div><div className="mb-3 grid gap-2 rounded-md border border-amber-200 bg-amber-50 p-2 text-xs text-amber-900 md:grid-cols-3"><div><span className="font-semibold">Business problem:</span> {row.problem}</div><div><span className="font-semibold">Why it needs review:</span> {row.why_not_automatic}</div><div><span className="font-semibold">Recommended action:</span> {row.recommended_action}</div></div><div className="grid gap-3 rounded-md border border-blue-200 bg-blue-50/40 p-3 sm:grid-cols-2 lg:grid-cols-4">
          {['expense','petty_cash','capital_contribution'].includes(row.document_type) && needsCategory && <label className="text-xs">Expense Category<select name="edit" aria-label="Edit" value={edit.expense_category || ''} onChange={event => updateEdit(row,'expense_category',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{expenseCategories.map(category => <option key={category.id} value={category.value}>{category.group !== category.label ? `${category.group} › ` : ''}{category.label}</option>)}</select></label>}
          {row.document_type === 'expense' && needsCategory && <label className="text-xs">Expense Subcategory<select name="edit" aria-label="Edit" value={edit.expense_subcategory || ''} onChange={event => updateEdit(row,'expense_subcategory',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option><option value={row.current_subcategory || ''}>{humanize(row.current_subcategory)}</option></select></label>}
          {needsSupplier && <label className="text-xs">Supplier<select name="edit" aria-label="Edit" value={edit.supplier_id || ''} onChange={event => updateEdit(row,'supplier_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{suppliers.map(item => <option key={item.id} value={item.id}>{item.company_name}</option>)}</select></label>}
          {needsCustomer && <label className="text-xs">Customer<select name="edit" aria-label="Edit" value={edit.customer_id || ''} onChange={event => updateEdit(row,'customer_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{customers.map(item => <option key={item.id} value={item.id}>{item.company_name}</option>)}</select></label>}
          {needsBank && ['expense','receipt','payment','tax_payment'].includes(row.document_type) && <label className="text-xs">Bank Account<select name="edit" aria-label="Edit" value={edit.bank_account_id || ''} onChange={event => updateEdit(row,'bank_account_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{banks.map(item => <option key={item.id} value={item.id}>{bankLabel(item)}</option>)}</select></label>}
          {row.document_type === 'fund_transfer' && <>{needsTransferBanks && <><label className="text-xs">From Bank<select name="edit" aria-label="Edit" value={edit.from_bank_account_id || ''} onChange={event => updateEdit(row,'from_bank_account_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{banks.map(item => <option key={item.id} value={item.id}>{bankLabel(item)}</option>)}</select></label><label className="text-xs">To Bank<select name="edit" aria-label="Edit" value={edit.to_bank_account_id || ''} onChange={event => updateEdit(row,'to_bank_account_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{banks.map(item => <option key={item.id} value={item.id}>{bankLabel(item)}</option>)}</select></label></>}{needsFx && <label className="text-xs">Exchange Rate<input name="edit" aria-label="Required for FX conversion" type="number" min="0.000001" step="0.000001" value={edit.exchange_rate || ''} onChange={event => updateEdit(row,'exchange_rate',event.target.value)} placeholder="Required for FX conversion" className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}</>}
          {needsGl && <label className="text-xs">Chart of Accounts<select name="edit" aria-label="Edit" value={edit.account_id || ''} onChange={event => updateEdit(row,'account_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{coa.map(item => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></label>}
          {['expense','payment'].includes(row.document_type) && needsPayment && <label className="text-xs">Payment Method<input name="edit" aria-label="{row.current_payment_type || Select payment method}" value={edit.payment_type || ''} onChange={event => updateEdit(row,'payment_type',event.target.value)} placeholder={row.current_payment_type || 'Select payment method'} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}
          {['expense','payment'].includes(row.document_type) && needsTax && <label className="text-xs">Tax Category<select name="edit" aria-label="Edit" value={edit.tax_code_id || ''} onChange={event => updateEdit(row,'tax_code_id',event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage}><option value="">No change</option>{taxCodes.map(item => <option key={item.id} value={item.id}>{item.code} — {item.name}</option>)}</select></label>}
          {['expense','receipt','payment'].includes(row.document_type) && needsReference && <label className="text-xs">Reference<input name="edit" aria-label="{row.current_reference || No change}" value={edit.reference || ''} onChange={event => updateEdit(row,'reference',event.target.value)} placeholder={row.current_reference || 'No change'} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}
          {row.document_type === 'bank_reconciliation' && row.current_bank_account_id && <div className="sm:col-span-2 lg:col-span-3"><BankTransactionLinkField
            ...
            disabled={!canManage}
          /></div>}
          {row.document_type === 'journal' && <label className="text-xs">Document Classification<input name="edit" aria-label="No change" value={edit.document_classification || ''} onChange={event => updateEdit(row,'document_classification',event.target.value)} placeholder="No change" className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}
          {['loan','capital_contribution'].includes(row.document_type) && <label className="text-xs">Finance Classification<input name="edit" aria-label="{row.current_finance_classification || No change}" value={edit.finance_classification || ''} onChange={event => updateEdit(row,'finance_classification',event.target.value)} placeholder={row.current_finance_classification || 'No change'} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}
          {['sales_invoice','purchase_invoice'].includes(row.document_type) && <label className="text-xs">Faktur Pajak Number<input name="edit" aria-label="{row.current_faktur_pajak_number || No change}" value={edit.faktur_pajak_number || ''} onChange={event => updateEdit(row,'faktur_pajak_number',event.target.value)} placeholder={row.current_faktur_pajak_number || 'No change'} className="mt-1 w-full rounded border px-2 py-1.5" disabled={!canManage} /></label>}
