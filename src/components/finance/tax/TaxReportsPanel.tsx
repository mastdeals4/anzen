    ...
    case 'pph_register': {
      ...
      const imports = (importRes.data ?? []).map((r: any) => {
        ...
        return {
          ...
          'Gross Amount (Rp)': gross,
          'DPP (Rp)': gross,
          'Tax Classification': 'Prepaid / Advance Tax (Kredit Pajak PPh 22)',
          'PPh Amount (Rp)': pph,
          'Net Paid (Rp)': net,
          'Payment Method': 'Bank Transfer (PIB)',
          'Status': r.approval_status === 'approved' ? 'Approved (Paid at Customs)' : 'Pending Approval',
          'Filing Readiness': 'Annual Return (SPT Tahunan PPh Badan)',
          'Notes': r.description || '',
        };
      });
