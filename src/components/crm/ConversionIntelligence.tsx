...
export function ConversionIntelligence() {
  ...
  return <div className="space-y-3"><div className="flex flex-wrap items-center gap-2"><div><h2 className="text-lg font-semibold">Conversion Intelligence</h2><p className="text-sm text-gray-600">Evidence only. Signals are prompts for review, never confirmed reasons.</p></div><label className="ml-auto flex items-center gap-2 text-sm"><input name="checkbox" aria-label="Checkbox" type="checkbox" checked={onlyNeverBought} onChange={e => setOnlyNeverBought(e.target.checked)} />Asked but never bought</label><input name="filter" aria-label="Customer or product" value={filter} onChange={e => setFilter(e.target.value)} placeholder="Customer or product" className="border rounded px-2 py-1 text-sm" /></div>
