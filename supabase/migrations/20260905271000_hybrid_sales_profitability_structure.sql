...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public,pg_temp AS $$
WITH x AS (
 SELECT ar.line_id,ar.invoice_id,si.invoice_number,si.invoice_date,si.customer_id,COALESCE(c.company_name,'Unknown Customer') customer_name,si.sales_order_id,so.so_number,
   dci.challan_id dc_id,dc.challan_number dc_number,ar.quantity,sii.unit_price selling_price,ROUND(ar.quantity*sii.unit_price,2) gross_sales,
   ar.authoritative_unit_cogs unit_cost,ar.authoritative_cogs line_cost,COALESCE(le.sales_expense,0) line_sales_expense,ar.resolution_tier cogs_resolution
 ...
 LEFT JOIN public.customers c ON c.id=si.customer_id LEFT JOIN public.sales_orders so ON so.id=si.sales_order_id LEFT JOIN public.delivery_challan_items dci ON dci.id=sii.delivery_challan_item_id LEFT JOIN public.delivery_challans dc ON dc.id=dci.challan_id
 ...
 SELECT fe.delivery_challan_id dc_id,jsonb_agg(jsonb_build_object('id',fe.id,'voucher_number',fe.voucher_number,'category',fe.expense_category,'total_amount',fe.amount,'description',fe.description,'expense_date',fe.expense_date)) expenses
 FROM public.finance_expenses fe WHERE fe.expense_category IN ('delivery_sales','loading_sales') AND fe.approval_status='approved' AND fe.delivery_challan_id IN (SELECT DISTINCT dc_id FROM x WHERE dc_id IS NOT NULL) GROUP BY fe.delivery_challan_id
)
