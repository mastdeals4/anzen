-- Sales profitability must use the batch's effective historical cost.
-- Local batches (no import container) are costed from import_price_per_unit;
-- imported batches require a positive landed_cost_per_unit. No data is changed.

CREATE OR REPLACE FUNCTION public.effective_sales_cogs_unit_cost(p_batch_id uuid)
RETURNS numeric
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT CASE
    WHEN b.import_container_id IS NULL
      THEN NULLIF(b.import_price_per_unit, 0)
    WHEN b.landed_cost_per_unit > 0
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH lines AS (
    ...
    SELECT product_id, product_name, product_code, product_unit,
      SUM(quantity) AS total_qty_sold, SUM(quantity * unit_price) AS total_revenue,
  ...
  SELECT *, ROUND(COALESCE(costed_revenue, 0) - total_cogs, 2) AS total_profit,
...
CREATE OR REPLACE FUNCTION public.get_sales_profit_drilldown(p_product_id uuid, p_start_date date, p_end_date date)
RETURNS TABLE (
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT si.id, si.invoice_number, si.invoice_date, COALESCE(c.company_name, ''),
    ...
    ec.effective_unit_cost,
    ROUND(sii.quantity * sii.unit_price, 2),
    CASE WHEN ec.effective_unit_cost IS NOT NULL THEN ROUND(sii.quantity * ec.effective_unit_cost, 2) END,
    CASE WHEN ec.effective_unit_cost IS NOT NULL THEN ROUND(sii.quantity * (sii.unit_price - ec.effective_unit_cost), 2) END,
    CASE WHEN ec.effective_unit_cost IS NOT NULL AND sii.unit_price <> 0
      THEN ROUND((sii.unit_price - ec.effective_unit_cost) / sii.unit_price * 100, 2) END,
    ec.effective_unit_cost IS NULL
  FROM public.sales_invoice_items sii
  JOIN public.sales_invoices si ON si.id = sii.invoice_id
  LEFT JOIN public.customers c ON c.id = si.customer_id
  JOIN public.products p ON p.id = sii.product_id
  LEFT JOIN public.batches b ON b.id = sii.batch_id
  LEFT JOIN LATERAL (SELECT public.effective_sales_cogs_unit_cost(sii.batch_id) AS effective_unit_cost) ec ON true
...
RETURNS TABLE (month_label text, month_start date, total_sales numeric, total_orders bigint,
  ...
  profit_pct numeric, outbound_delivery numeric, operational_profit numeric,
  costed_lines bigint, total_lines bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH lines AS (
    SELECT si.id, si.invoice_date, sii.quantity, sii.unit_price,
      public.effective_sales_cogs_unit_cost(sii.batch_id) AS effective_unit_cost
    ...
    SELECT date_trunc('month', invoice_date)::date AS month_start,
      ...
      SUM(quantity * unit_price) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_revenue,
      COALESCE(SUM(quantity * effective_unit_cost) FILTER (WHERE effective_unit_cost IS NOT NULL), 0) AS total_cogs,
      COUNT(*) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_lines, COUNT(*) AS total_lines
    ...
    FROM public.finance_expenses WHERE expense_date >= p_start_date AND expense_date <= p_end_date GROUP BY 1
  )
  SELECT m.month_label, m.month_start, ROUND(m.total_sales, 2), m.total_orders, m.total_qty_sold,
    ROUND(m.total_sales / NULLIF(m.total_orders, 0), 2), ROUND(m.total_cogs, 2),
    ROUND(COALESCE(m.costed_revenue, 0) - m.total_cogs, 2),
    CASE WHEN COALESCE(m.costed_revenue, 0) = 0 THEN NULL ELSE ROUND((m.costed_revenue - m.total_cogs) / m.costed_revenue * 100, 2) END,
...
CREATE OR REPLACE FUNCTION public.get_product_performance_report(p_start_date date, p_end_date date)
RETURNS TABLE (product_id uuid, product_name text, product_code text, qty_sold numeric,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH lines AS (
    SELECT sii.product_id, p.product_name, COALESCE(p.product_code, '') AS product_code,
      sii.quantity, sii.unit_price, public.effective_sales_cogs_unit_cost(sii.batch_id) AS effective_unit_cost
    FROM public.sales_invoice_items sii JOIN public.sales_invoices si ON si.id = sii.invoice_id
      JOIN public.products p ON p.id = sii.product_id
    WHERE si.invoice_date >= p_start_date AND si.invoice_date <= p_end_date AND COALESCE(si.is_draft, false) = false
  ), grouped AS (
    SELECT product_id, product_name, product_code, SUM(quantity) AS qty_sold, SUM(quantity * unit_price) AS total_sales,
      COALESCE(SUM(quantity * effective_unit_cost) FILTER (WHERE effective_unit_cost IS NOT NULL), 0) AS total_cost,
      SUM(quantity * unit_price) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_revenue,
      COUNT(*) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_lines, COUNT(*) AS total_lines
  ...
  SELECT product_id, product_name, product_code, qty_sold, ROUND(total_sales, 2), ROUND(total_cost, 2),
    ...
    CASE WHEN COALESCE(costed_revenue, 0) = 0 THEN NULL ELSE ROUND((costed_revenue - total_cost) / costed_revenue * 100, 2) END,
    costed_lines, total_lines, ROUND(costed_lines::numeric / NULLIF(total_lines, 0) * 100, 2)
  FROM grouped ORDER BY total_sales DESC;
$$;

CREATE OR REPLACE FUNCTION public.get_customer_sales_report(p_start_date date, p_end_date date)
RETURNS TABLE (customer_id uuid, customer_name text, total_orders bigint, total_sales numeric,
  avg_order_value numeric, last_order_date date, total_qty numeric, total_cogs numeric,
  total_profit numeric, profit_pct numeric, costed_lines bigint, total_lines bigint, cost_coverage numeric)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH lines AS (
    ...
    SELECT customer_id, company_name, COUNT(DISTINCT id) AS total_orders, SUM(quantity * unit_price) AS total_sales,
      SUM(quantity) AS total_qty, SUM(quantity * unit_price) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_revenue,
      COALESCE(SUM(quantity * effective_unit_cost) FILTER (WHERE effective_unit_cost IS NOT NULL), 0) AS total_cogs,
      MAX(invoice_date) AS last_order_date, COUNT(*) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_lines, COUNT(*) AS total_lines
    FROM lines GROUP BY customer_id, company_name
  )
  SELECT customer_id, company_name, total_orders, ROUND(total_sales, 2), ROUND(total_sales / NULLIF(total_orders, 0), 2), last_order_date,
    total_qty, ROUND(total_cogs, 2), ROUND(COALESCE(costed_revenue, 0) - total_cogs, 2),
    CASE WHEN COALESCE(costed_revenue, 0) = 0 THEN NULL ELSE ROUND((costed_revenue - total_cogs) / costed_revenue * 100, 2) END,
    costed_lines, total_lines, ROUND(costed_lines::numeric / NULLIF(total_lines, 0) * 100, 2)
  FROM grouped ORDER BY total_sales DESC;
$$;

CREATE OR REPLACE FUNCTION public.get_expense_vs_profit_report(p_start_date date, p_end_date date)
RETURNS TABLE (total_sales numeric, total_cogs numeric, gross_profit numeric, outbound_delivery numeric,
  contribution_profit numeric, operating_expenses numeric, operational_profit numeric, profit_pct numeric,
  costed_lines bigint, total_lines bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH lines AS (
    SELECT sii.quantity, sii.unit_price, public.effective_sales_cogs_unit_cost(sii.batch_id) AS effective_unit_cost
    FROM public.sales_invoices si JOIN public.sales_invoice_items sii ON sii.invoice_id = si.id
    WHERE si.invoice_date >= p_start_date AND si.invoice_date <= p_end_date AND COALESCE(si.is_draft, false) = false
  ), sales AS (
    SELECT COALESCE(SUM(quantity * unit_price), 0) AS total_sales,
      COALESCE(SUM(quantity * unit_price) FILTER (WHERE effective_unit_cost IS NOT NULL), 0) AS costed_revenue,
      COALESCE(SUM(quantity * effective_unit_cost) FILTER (WHERE effective_unit_cost IS NOT NULL), 0) AS total_cogs,
      COUNT(*) FILTER (WHERE effective_unit_cost IS NOT NULL) AS costed_lines, COUNT(*) AS total_lines FROM lines
  ), expenses AS (
  ...
  SELECT ROUND(s.total_sales, 2), ROUND(s.total_cogs, 2), ROUND(s.costed_revenue - s.total_cogs, 2), ROUND(e.outbound_delivery, 2),
    ROUND(s.costed_revenue - s.total_cogs - e.outbound_delivery, 2), ROUND(e.operating_expenses, 2),
    ROUND(s.costed_revenue - s.total_cogs - e.outbound_delivery - e.operating_expenses, 2),
