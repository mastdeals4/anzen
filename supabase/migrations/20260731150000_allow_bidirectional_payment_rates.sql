-- Allow canonical payment rates in either direction.
-- A rate is always: 1 invoice-currency unit = N bank-currency units.
-- Therefore USD -> IDR is > 1 while IDR -> USD is normally < 1.
...
BEGIN
  ...
  IF v_rate <= 0 THEN
    RAISE EXCEPTION 'Exchange rate must be positive for % to % payment', v_invoice_currency, v_bank_currency;
  END IF;
