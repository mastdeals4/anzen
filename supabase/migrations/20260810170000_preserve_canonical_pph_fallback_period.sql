-- Manual PPh period selection is opt-in. Untouched documents retain the
-- established payment/due-date attribution helper used by the tax engine.
BEGIN;
...
BEGIN
  ...
  SELECT
    COALESCE((SELECT SUM(fe.pph_amount) FROM public.finance_expenses fe LEFT JOIN public.tax_codes tc ON tc.id=fe.pph_code_id
      WHERE (fe.pph_tax_period_id=v_period.id OR (fe.pph_tax_period_id IS NULL AND EXTRACT(YEAR FROM public.get_expense_pph_period_date(fe.id))::int=v_period.fiscal_year AND EXTRACT(MONTH FROM public.get_expense_pph_period_date(fe.id))::int=v_period.period_month))
        AND fe.approval_status='approved' AND fe.pph_amount>0 AND COALESCE(fe.expense_category,'') NOT IN ('pib_import','pph_import')
        AND (v_period.tax_type='PPh_Unifikasi' OR tc.tax_type=v_period.tax_type)),0)
    ...
    + CASE WHEN v_period.tax_type IN ('PPh22','PPh_Unifikasi') THEN COALESCE((
      SELECT SUM(CASE WHEN fe.expense_category='pib_import' THEN fe.pib_pph_amount ELSE fe.amount END) FROM public.finance_expenses fe
      WHERE fe.expense_category IN ('pib_import','pph_import')
        AND (fe.pph_tax_period_id=v_period.id OR (fe.pph_tax_period_id IS NULL AND EXTRACT(YEAR FROM public.get_expense_pph_period_date(fe.id))::int=v_period.fiscal_year AND EXTRACT(MONTH FROM public.get_expense_pph_period_date(fe.id))::int=v_period.period_month))
        AND fe.approval_status='approved'),0) ELSE 0 END
  INTO v_pph_total;
