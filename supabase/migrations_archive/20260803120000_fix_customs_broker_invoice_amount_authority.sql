-- Approved Finance V1.1.2 freeze exception: Customs Broker calculation only.
--
-- Invoice Amount is the actual supplier payable. DPP and PPN are independent
-- tax-reporting values and must not be added to that payable. Historical rows
-- with an empty/zero Invoice Amount retain their legacy DPP + PPN fallback.
-- Every line created or edited by the corrected UI carries
-- invoice_amount_authoritative=true, making even a stored zero authoritative.
...
AS $$
  SELECT CASE
    WHEN COALESCE((p_item->>'invoice_amount_authoritative')::boolean, false)
      THEN COALESCE((p_item->>'amount')::numeric, 0)
    WHEN COALESCE((p_item->>'amount')::numeric, 0) <> 0
      THEN COALESCE((p_item->>'amount')::numeric, 0)
    ELSE COALESCE((p_item->>'dpp_amount')::numeric, 0)
...
IMMUTABLE
AS $$
  SELECT public.broker_reimbursement_line_total(p_item)
       - COALESCE((p_item->>'ppn_amount')::numeric, 0)
$$;
...
BEGIN
  ...
  v_line_total := COALESCE(NEW.amount,0) + COALESCE(NEW.stamp_duty_amount,0) - v_pph;
  IF v_line_total<>0 THEN
    INSERT INTO journal_entry_lines(journal_entry_id,line_number,account_id,description,debit,credit,supplier_id)
