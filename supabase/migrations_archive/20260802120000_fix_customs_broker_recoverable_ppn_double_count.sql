-- Customs Broker accounting formula correction.
--
-- Reimbursement line totals are gross and already include line PPN. The
-- canonical model previously added that PPN again to cash/AP and also debited
-- the gross reimbursement to expense before debiting Recoverable PPN.
--
-- This migration changes formulas only. It does not update source documents,
-- historical journals, bank links, or reconciliation controls.
...
BEGIN
  ...
  -- Line PPN is already inside v_reimbursement. Only header PPN remains additive.
  v_credit_total := v_expense_total + COALESCE(NEW.ppn_amount,0);
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(COALESCE(NEW.broker_items,'[]'::jsonb)) LOOP
    ...
    v_line_total := COALESCE((v_item->>'amount')::numeric,0)
      + COALESCE((v_item->>'dpp_amount')::numeric,0);
  ...
  v_line_total := COALESCE(NEW.amount,0)
